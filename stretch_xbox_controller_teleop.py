...
import stretch_body.bridge_test as xc
...
############################
# Regular Motion
dead_zone = 0.25  # 0.25 #0.1 #0.2 #0.3 #0.4
move_s = 0.3
...
move_s = 0.05
max_dist_rad = 0.09  # 0.2 #0.25 #0.1 #0.09
accel_rad = 0.25  # 0.05 # increase acceleration of turning from 0.03 to .25
...
def manage_lift_arm(robot, controller_state):
    ...
    if abs(lift_command) > dead_zone:
        ...
        d_m, v_m, a_m = command_to_linear_motion.get_dist_vel_accel((output_sign * .6), lift_command)
        robot.lift.move_by(d_m, v_m, a_m)

    if abs(arm_command) > dead_zone:
        output_sign = math.copysign(1, arm_command)
        d_m, v_m, a_m = command_to_linear_motion.get_dist_vel_accel((output_sign * .6), arm_command)
...
def manage_rotate(robot, controller_state):
    rotate = controller_state['left_button_pressed']
    if rotate:
        fast_navigation_mode = False
        navigation_mode_trigger = controller_state['right_trigger_pulled']
        if navigation_mode_trigger > 0.5:
        ...
        if abs(turn_command) > dead_zone:
            ...
            if not fast_navigation_mode:
                ...
                d_rad, v_rad, a_rad = fast_command_to_rotary_motion.get_dist_vel_accel(output_sign, turn_command)
            robot.base.rotate_by(d_rad - 1.5, v_rad, a_rad - 1.5)


def manage_tinyforward(robot, controller_state):
    forward = controller_state['top_pad_pressed']
    # --------------------------------------------------------------------------
    if forward:
        fast_navigation_mode = False
        navigation_mode_trigger = controller_state['right_trigger_pulled']
        ...
        if abs(forward_command) > dead_zone:
            ...
            if not fast_navigation_mode:
                ...
                d_m, v_m, a_m = fast_command_to_linear_motion.get_dist_vel_accel(output_sign, forward_command)
            robot.base.translate_by(d_m + .05, v_m + 1, a_m + 1)
    ...
    try:
        ...
        while True:
            ...
            if not robot.is_calibrated():
                ...
                manage_rotate(robot, controller_state)
                manage_tinyforward(robot, controller_state)
